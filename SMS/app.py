...
set_default_color_theme(os.path.join(current_dir, "assets","breeze.json"))
...
def Main():
    ...
    def add_record():
        ...
        # Ensure index number is only numbers
        if not student_id.isdigit():
            errors.append("Index number must contain only numbers.")

        if student_id in student_records:
            errors.append("Record with this Index number already exists.")
        ...
        try:
            ...
            errors.append("Marks must be numeric values.")

        # Midsem limits
        if not (0 <= be_midsem <= 30):
            errors.append("BE Midsem marks must be between 0 and 30.")
        if not (0 <= ee_midsem <= 30):
            errors.append("EE Midsem marks must be between 0 and 30.")
        if not (0 <= calc_midsem <= 30):
            errors.append("Calculus Midsem marks must be between 0 and 30.")

        # Exam limits
        if not (0 <= be_exam <= 70):
            errors.append("BE exam marks must be between 0 and 70.")
        if not (0 <= ee_exam <= 70):
            errors.append("EE exam marks must be between 0 and 70.")
        if not (0 <= calc_exam <= 70):
            errors.append("Calculus exam marks must be between 0 and 70.")

        if errors:
            messagebox.showerror("Score Input Error", "\n".join(errors))
            return
